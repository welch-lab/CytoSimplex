...
Technically, any forms of feature-by-observation matrix is acceptable for the method we developed, and users are encouraged to explore the usability of our method with other types of data, even not biological. However, single-cell transcriptomics data, as provided, usually is of high dimensionality and contains technical and biological noise. Meanwhile, distance calculation tends to be computationally expensive with such high dimensionality. With testing different approaches of reducing the dimensionality and noise, we recommend that users select a number of top differentially expressed genes (DEGs) for each cluster that a vertex represents. 
...
```{R selectTopGenes}
vertices <- c("OS", "RE", "CH")
geneSelect <- selectTopFeatures(rnaRaw, clusterVar = rnaCluster, vertices = vertices,
                             nTop = 30, normalize = TRUE)
head(geneSelect)

stats <- selectTopFeatures(rnaRaw, clusterVar = rnaCluster, vertices = vertices,
                        nTop = 30, normalize = TRUE, returnStats = TRUE)
head(stats)
```

## Simplex visualization

We provide three core functions, as demonstrated below.

1. `plotBinary()` - Shows sample similarity in binary simplex -- two-ended line. In practice, we jittered the y-axis of the plot in order to clearly show where the dots are. The closer a dot, a cell, is to one end (left or right vertex), the more similar the cell is to the cell cluster that vertex represents. A histogram density line is added as another layer to show the distribution of cells across the horizontal simplex.

```{R plotBinary, fig.width = 5, fig.height = 3}
vertices <- c("OS", "RE")
rnaLognorm <- colNormalize(rnaRaw, scaleFactor = 1e4, log = TRUE)
binFeatures <- selectTopFeatures(rnaLognorm, clusterVar = rnaCluster, 
                              vertices = vertices, nTop = 30,
                              normalize = FALSE)
plotBinary(rnaLognorm[binFeatures, ], rnaCluster, vertices[1:2])
```

2. `plotTernary()` - Shows sample similarity in ternary simplex -- equilateral triangle. The closer a dot, a cell, is to one angle (left, top or right vertex), the more similar the cell is to the cell cluster that vertex represents. 

```{R plotTernary, fig.height = 4, fig.width = 4}
vertices <- c("OS", "RE", "CH")
ternFeatures <- selectTopFeatures(rnaLognorm, clusterVar = rnaCluster, 
                               vertices = vertices, nTop = 30,
                               normalize = FALSE)
plotTernary(rnaLognorm[ternFeatures, ], rnaCluster, vertices)
```
...
3. `plotQuaternary()` - Shows sample similarity in quaternary simplex -- tetrahedron. The 3D visualization denpends on package `plot3D`. For quaternary simplex, we need one more cluster as a vertex. Here, we add the cells in osteoblast-reticular transition state (`"ORT"`) into the vertex list.

```{R plotQuaternary, fig.height = 4, fig.width = 4}
vertices <- c("OS", "RE", "CH", "ORT")
quatFeature <- selectTopFeatures(rnaLognorm, clusterVar = rnaCluster, 
                              vertices = vertices, nTop = 30,
                              normalize = FALSE)
plotQuaternary(rnaLognorm[quatFeature, ], rnaCluster, vertices)
```
...
library(patchwork)
vertices <- c("OS", "RE", "CH")
ternList <- plotTernary(rnaLognorm[ternFeatures, ], rnaCluster, vertices, splitCluster = TRUE)
...
veloSplit <- plotTernary(rnaLognorm[ternFeatures, ], rnaCluster, vertices, 
